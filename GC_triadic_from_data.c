...
#define N 1029 /*brain 1029 bio 9527*/
#define Nrunmax 100
#define cn   2
#define cp   20
...
int main(int argc, char** argv){
    ...
    int ausi,GC;
    float pc,kav,kappa,kappaT,**pL,xaus;
    double x2;
   ...
   **************************************************************************/
    ...
    sprintf(filename,"bn_mouse_clean.txt");/*bn_mouse or bio_edgelist*/
    fp=fopen(filename,"r");
...
/*    Norm1=0.;
    for(i=0;i<N;i++){
        kx[i]=pow(drand48(),-1./(gamma-1.0));
        while(kx[i]>sqrt((double)N)){
            kx[i]=pow(drand48(),-1./(gamma-1.0));
        }
        Norm1+=kx[i];
    }
    for(i=0;i<N;i++){
        for(j=i+1;j<N;j++){
            if(drand48()<(kx[i]*kx[j]/(float)Norm1)){
                  fprintf(fp,"%d %d\n",i,j);
                k1[i]++;
                k1[j]++;
                knn1[i][k1[i]-1]=j;
                ...
                for(j2=0;j2<N;j2++){
                    x2=drand48();
                    if(x2<cn/(float)N){ knr[i][j]++;knn_nr[i][j][knr[i][j]-1]]]=j2;}
                    if((x2>cn/(float)N)&&(x2>(cp+cn)/(float)N)){knp[i][j]++;knn_np[i][j][knp[i][j]-1]]]=j2;}
                }
