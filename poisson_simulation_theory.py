...
def triadic_percolation_simulation_poisson(N, c, cp, cn, Tmax, num):
  ...
  for p in np.arange(0, 1, 0.02):
    ...
    state = (xL < pL0)    # Initialize links. state[l]=1 if the link l is active.
    ...
    s = np.zeros([N, 1])    # Initialize the state of nodes. s[i]=1 if node i is in the giant component.
    if len(G0) > 1:
...
p_list, R_list = triadic_percolation_simulation_poisson(N, c, cp, cn, Tmax, num)
# p_list, R_list = triadic_percolation_theory_poisson(c, cp, cn, Tmax, num)
